...
def get_obs(env, tmpair, allstation):
    now_time = env.now
    if now_time >= 0 and now_time < pulse:
        station_id = 0
    elif now_time >= pulse and now_time < 2 * pulse:
...
def generate_episode(agents, conf,pulses,episode_num, evaluate=False):
        env = simpy.Environment()
        ##TODO:
        #   o:每个agent的状态的合集
        #   u：所有agent的动作合集
        ...
        obs, _ = get_obs(env, air, allstation)
        ...
        while n_finish < pro_num:
            ...
            for i in range(recurrent_num):
                ...
                freeo.remove(new_o)
                if order_stfi[1] > pulse * station_id:
        ...
        def production(env, agents, air, allstation, station_list, evaluate, start_epsilon):
            ...
            while not done:
                ...
                next_obs,done = get_states(env,air,allstation)
