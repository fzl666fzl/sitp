...
# pulse = 660
action_set = [[1,0,0],[0.8,0.2,0],[0.3,0.5,0.2],
...
def get_reward(now_time, allstation, air,thispulse):
    ...
    # st_time = [nowstation.teams[i].finishtime for i in range(team_num)]
    st_time = [nowstation.time_past for i in range(team_num)]
...
def get_pulse(allstation):
    ...
    for i in range(station_num):
        ...
        cts.append(time_tmp_station)
        meanct = meanct + time_tmp_station
    meanct = meanct/station_num
    for i in range(station_num):
        sii += (cts[0] - meanct)**2
    sii = math.sqrt(sii/station_num)
    return pulse_real,sii
...
def generate_episode(agents, conf,pulses,thispulse,episode_num, actionss,siis,evaluate=False):
        env = simpy.Environment()
        o, u, r, s, o_,s_ = [], [], [], [], [], []
        ...
        def production(env, agents, air, allstation, station_list, evaluate, start_epsilon):
            ...
            epsilon = 0 if episode_num>int(conf.n_epochs*0.99) else conf.start_epsilon
