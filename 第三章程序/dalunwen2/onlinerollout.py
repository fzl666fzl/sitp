...
def get_reward(now_time, allstation, air,thispulse):
    ...
    if(rew<550):rew = -0.9
    # elif(rew>=7.0 and rew<7.1): rew = 0
    elif(rew >= 550 and rew<580): rew = 0.5
    elif (rew >= 580 and rew < 590):rew = 0.7
...
def reset_env(env,thispulse):
    ...
    allstation = []
    isfirsttongji = []
    thispulses = [608,650,650,650,650]

    for k in range(station_num):
        allstation.append(Station(env, k, 1,thispulses[k]))
...
def generate_episode(agents, conf, pulses, thispulse, episode_num, actionss, evaluate=False):
    ...
    def production(env, agents, air, allstation, station_list, evaluate, start_epsilon):
        ...
        bi = 0
        while not done:
            obs, _ = get_obs(env, air, allstation, tps[tpi])
            ...
            #     actions.append(best_action[bi])
            #     bi += 1

            for agent_id in range(n_agents):
                avail_action = action_set
                # if bi < 2:
                #     action = best_action[bi]
                #     actions.append(best_action[bi])
                #     bi += 1
                # else:
            ...
            next_state, done = get_states(env, air, allstation, tps[tpi])
            next_obs, done = get_states(env, air, allstation, tps[tpi])
            reward = get_reward(env.now, allstation, air, tps[tpi])
