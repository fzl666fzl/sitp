...
# import rollout1130
import onlinerollout_dis
import simpy
from onlinerollout_dis import dis_o
from onlinerollout_dis import dis_tw
...
def train():
    ...
    for epoch in range(1):
        ...
        if not pulses:
            ...
            now_pulse = min(int(min(pulses)-1),now_pulse) if now_pulse > 270 else 270


        pulses = []
        for episode_idx in range(1):##n_eposodes=1
            episode, times, pp = onlinerollout_dis.generate_episode(agents, conf,pulses,now_pulse,epoch,SI)
    ...
    print(SI)
    df_diso = pd.DataFrame(dis_o)
    df_tw = pd.DataFrame(dis_tw)
    # df_diso.to_excel('扰动工序固定.xlsx')
    # df_tw.to_excel('扰动概率固定.xlsx')
