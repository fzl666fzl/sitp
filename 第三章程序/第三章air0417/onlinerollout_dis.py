    ...
    def calrule(self,air,nowrule):
        ...
        while air.orders_free:
            ...
            if self.id < station_num - 1:
                # tw = random.random()
                # if tw < 0.2:
                #     dis_o.append(thisorder)
                #     dis_tw.append(round(tw,2))
                #     tmptime += dict_time[thisorder] * tw

                if thisorder in dis_o:
                    # dis_o.append(thisorder)
                    # print(thisorder)
                    tmptime += dict_time[thisorder] * dict_dis[thisorder]
            if dict_free[thisorder] == 1:  ####初始的自由工序
                order_stfi[0] = latestfi
                order_stfi[1] = order_stfi[0] + dict_time[thisorder] * maxefi + tmptime
            else:
                tmp_maxtime = 0
            ...
            for k in range(thisnum):
                ...
                thisworkers[k].timelist[thisorder] = dict_time[thisorder] * maxefi + tmptime
                thisworkers[k].time_past += dict_time[thisorder] * maxefi + tmptime
        ...
        while not done:
            ...
            if nowstation.id == station_num - 1:
                ...
                for i in range(station_num):
                    ...
                    if this_pulse <= 290 and this_pulse > 280:
                        tmp = 0.5
                    elif this_pulse <= 280 and this_pulse > 275:
                        tmp = 0.7
                    elif this_pulse < 275:
                        tmp = 1
                    elif this_pulse <= 300 and this_pulse > 290:
                        tmp = 0.3
                    else:
                        tmp = -0.1
