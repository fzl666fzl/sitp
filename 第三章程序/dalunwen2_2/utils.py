        ...
        while not terminated and step < self.episode_limit:
            ...
            for agent_id in range(self.n_agents):
                ...
                action = self.agents.choose_action(
                    obs[agent_id], last_action[agent_id], agent_id, avail_action, epsilon, evaluate
                )
...
class ReplayBuffer:
    def __init__(self, conf):
        ...
        if getattr(self.conf, "verbose", False):
            ...
            print(self.buffers["u_onehot"].shape)

    def store_episode(self, episode_batch):
        batch_size = len(episode_batch["o"])
        with self.lock:
            idxs = self._get_storage_idx(inc=batch_size)
            if getattr(self.conf, "verbose", False):
                print("当前在buffer里面的位置为", idxs)
