...
# pulse = 660
action_set = [[1,0,0],[0.8,0.2,0],[0.5,0,0.4],
...
def get_pulse(allstation):
    ...
    return pulse_real,si,station_times
        ...
        def production(env, agents, air, allstation, station_list, evaluate, start_epsilon):
            ...
            while not done:
                ...
                if nowstation.id == station_num-1:
                    this_pulse,times,station_times = get_pulse(allstation)
                    pulses.append(this_pulse)
                    if this_pulse <= thispulse:
                        SI.append(times)
                        print("平滑指数是：", times)
