...
def main():
    tag = os.getenv("END2END_EVAL_TAG", "balanced_best")
    seeds = parse_seeds(os.getenv("END2END_EVAL_SEEDS", "100-109"))
    init_pulse = int(os.getenv("END2END_EVAL_INIT_PULSE", "608"))
    ...
    for seed in seeds:
        ...
        print(
            ...
            "finished={finished}".format(
                ...
                finished=summary.get("finished_order_count"),
            )
        )
    ...
    finished = np.array([item.get("finished_order_count", 0) for item in results], dtype=int)
    print("tag =", tag)
    ...
    print("finished 50 =", int(np.sum(finished == rollout_end2end.pro_num)))
