        ...
        while order_free and team_free:
            ...
            if self.id < station_num - 1:
                if tmp_team_stfi[1] <= self.pulse * (self.id + 1):
                    ...
                    self.teams[t].time_past += dict_time[thisorder] * time_w + tmptime
                    self.teams[t].stfi[1] = tmp_team_stfi[1]
                    self.teams[t].timelist[thisorder] = dict_time[thisorder] * time_w + tmptime
        ...
        for tmpwhichstation in self.teams[team_id].order_buffer:
            ...
            if aircraft.isfirstprocedure[tmpwhichstation] == 1:  # 如果工序没有紧前，直接生产
                if aircraft.isfinish[tmpwhichstation] == 0:
                    ...
                    yield env.timeout(self.teams[team_id].timelist[tmpwhichstation])  # 生成延时时间，在这里停留该工序的装配时间
                    # print(f'实际工序：{tmpwhichstation}-站位：{self.id}-工作组:{team_id}-时间：{self.env.now}')
                    #
                    aircraft.finishtime[tmpwhichstation] = round(env.now, 2)
                    aircraft.team_id[tmpwhichstation] = team_id
                ...
                if aircraft.isfinish[tmpwhichstation] == 0:
                    ...
                    yield env.timeout(self.teams[team_id].timelist[tmpwhichstation])
                    # print(f'实际工序：{tmpwhichstation}-站位：{self.id}-工作组:{team_id}-时间：{self.env.now}')
                    aircraft.finishtime[tmpwhichstation] = round(env.now, 2)
                    self.teams[team_id].finishtime = round(env.now, 2)
                    aircraft.istrigger[tmpwhichstation - 1].succeed()
...
def get_reward(now_time, allstation, air,thispulse):
    ...
    # if(rew>=7.0):rew = -0.9
    # # elif(rew>=7.0 and rew<7.1): rew = 0
    # elif(rew>=6.8 and rew<7.0): rew = 0.5
    # elif (rew >= 6.5 and rew < 7.8):
    #     rew = 0.7
    # elif (rew >= 6.0 and rew < 6.5):rew = 0.9
    # # elif(rew>=6.0 and rew<6.5):rew = 2
    # else:rew = 0.1


    # if(rew>=6.9):rew = -0.9
    # # elif(rew>=7.0 and rew<7.1): rew = 0
    # elif(rew>=6.7 and rew<6.9): rew = 0.3
    # elif (rew >= 6.5 and rew < 6.7):
    #     rew = 0.7
    # elif (rew >= 6.0 and rew < 6.5):rew = 0.9
    # # elif(rew>=6.0 and rew<6.5):rew = 2
    # else:rew = 0.5
    ...
    # if(rew>=6.6):rew = -0.9
    # # elif(rew>=7.0 and rew<7.1): rew = 0
    # elif(rew>=6.4 and rew<6.6): rew = 0.5
    # elif (rew >= 6.2 and rew < 6.4):
    #     rew = 0.7
    # # elif (rew >= 6.0 and rew < 6.3):rew = 0.9
    # # elif(rew>=6.0 and rew<6.5):rew = 2
    # else:rew = 0.3

    rew = avg_st_time/1000
    return rew
...
def generate_episode(gene,thispulse,fits):
        env = simpy.Environment()
        ...
        def production(env, air, allstation, station_list):
            ...
            while not done:
                ...
                actions = gene[station_id*6:(station_id+1)*6]
                tmpactions = actions[:3]
                tmpa2 = actions[3:]
                actions = []
                actions.append(tmpactions)
                actions.append(tmpa2)
                ...
                if nowstation.id == station_num - 1:
                    ...
                    yield env.timeout(thispulse + 5000)
            # 最后一个动作




        env.process(production(env,air, allstation, station_list))
