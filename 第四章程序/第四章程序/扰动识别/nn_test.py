...
def data_processing(dataset,falg=False):#数据类型是pandas
    ...
    train_X, train_y = train[:, 0:8], train[:, -1:]  # 用前面的状态+性能预测后面的性能
    test_X, test_y = test[:, 0:8], test[:, -1:]
...
plt.plot(history1.history['val_accuracy'], label='原始数据')
plt.plot(history2.history['val_accuracy'], label='加SMOTE过采样')
plt.legend()
plt.xlabel("迭代次数")
plt.ylabel("准确率")
plt.show()
