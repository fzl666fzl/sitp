...
path = pandas.read_excel('扰动数据2.xlsx', header=0, index_col=0)
...
#模型训练
history1 = model.fit(train_X, train_label_onehot, epochs=50,validation_data=(test_X, test_label_onehot))
...
#模型训练
history1 = model.fit(train_X, train_label_onehot, epochs=80,validation_data=(test_X, test_label_onehot))
acc = history1.history['accuracy']
accs.append(acc[-1])
...
#模型训练
history1 = model.fit(train_X, train_label_onehot, epochs=100,validation_data=(test_X, test_label_onehot))
acc = history1.history['accuracy']
accs.append(acc[-1])
