...
# @Time : 2024.01.05
# @Author : wyl
# @Description : 此文件用于大论文第四章实验验证子策略验证的a部分，扰动识别方式的有效性验证
# -*- ecoding: utf-8 -*-
...
class Individual(object):
    ...
    def calculate_objective(self, allstation, airs,dis_station,dis_team,dis_worker,orderbufs,fintimes,timepsts,istwo,islast=False):


        if istwo:
            objs = reschedule1(allstation, airs, self.solution, dis_station, dis_team, dis_worker, orderbufs, fintimes,
                              timepsts, islast)
        else:
...
# TODO
def make_new_pop(P, eta, bound_min, bound_max,allstation, airs,dis_station,dis_team,dis_worker,orderbufs,fintimes,timepsts,istwo):
    """
    ...
    for i in range(int(popnum / 2)):
        ...
        Two_offspring = crossover_mutation(parent1, parent2, eta, bound_min, bound_max,allstation, airs, dis_station,dis_team,dis_worker,orderbufs,fintimes,timepsts,istwo)
...
def crossover_mutation(parent1, parent2, eta, bound_min, bound_max,allstation, airs,dis_station,dis_team,dis_worker,orderbufs,fintimes,timepsts,istwo):
    """
    ...
    # 计算目标函数值
    offspring1.calculate_objective(allstation, airs,dis_station,dis_team,dis_worker,orderbufs,fintimes,timepsts,istwo)
    offspring2.calculate_objective(allstation, airs,dis_station,dis_team,dis_worker,orderbufs,fintimes,timepsts,istwo)
...
def reschedule(allstation, airs, rule,dis_station,dis_team,dis_worker,orderbufs,fintimes,timepsts,islast=False):#rule是一个[0,0,0,0,0,0]指定了工序选择规则与工序分配规则
    ...
    for w in freeworkers:
        ...
        if now_p in thisorders:  ##说明正在加工的这道工序在等待别的工人，优先分配它，否则会死锁
            ...
            if dict_free[now_p] == 1:  ####初始的自由工序
                ...
                # order_stfi[1] = order_stfi[0] + dict_time[new_o] * maxefi
                order_stfi[1] = order_stfi[0] + dict_time[now_p]
            air.startingtime[now_p] = order_stfi[0]
    ...
    while freeo:
        ...
        for worker in tmpworkers:
            ...
            pri2 = 5 if worker.id == dis_worker else 1
            pri = [pri0, pri2]
        ...
        if virtual_flag:
            if virtual_starttime == 0:
                virtual_starttime = order_stfi[0]
            virtual_finishtime = order_stfi[1]
    ...
    fintime = 0
    for i in range(worker_num):
        for oo in thisteam.workers[i].order_buffer:
            if oo not in orderbufs[i]:
                distance += 1
        fintime = max(fintime, thisteam.workers[i].finishtime)

    return fintime,virtual_finishtime-virtual_starttime
def reschedule1(allstation, airs, rule,dis_station,dis_team,dis_worker,orderbufs,fintimes,timepsts,islast=False):#rule是一个[0,0,0,0,0,0]指定了工序选择规则与工序分配规则
    ##工人状态需要还原
    ...
    for w in freeworkers:
        ww = thisteam.workers[w]
        now_p = ww.order_buffer[ww.now_pro]##各工人当前正在加工的工序
        tmp_orders_finish.append(now_p)
        if now_p in thisorders:  ##说明正在加工的这道工序在等待别的工人，优先分配它，否则会死锁
            if dis_worker not in tmp_worlsts[now_p]:#若受扰动的工人不在待分配的工序工人列表中，则维持原调度
                new_w = tmp_worlsts[now_p][:]
                now_w = tmp_worlsts[now_p][:]
    ...
    while freeo:
        ...
        for n in nonfreeo:
            ffl = True
            for pre in dict_preorder[n]:
                if (pre not in tmp_orders_finish) and (dict_team[pre] == dis_team):
                    ffl = False
            if ffl:
                freeo.append(n)
                nonfreeo.remove(n)
        ...
        if dict_free[new_o] == 1:  ####初始的自由工序
            ...
            # order_stfi[1] = order_stfi[0] + dict_time[new_o] * maxefi
            order_stfi[1] = order_stfi[0] + dict_time[new_o]
        # print("所需要的人数", dict_workernum[new_o])
        for k in range(dict_workernum[new_o]):
            # thisworkers[k].time_past += dict_time[new_o] * maxefi
            # thisworkers[k].finishtime += dict_time[new_o] * maxefi
    ...
    for i in range(worker_num):
        for oo in thisteam.workers[i].order_buffer:
            if oo not in orderbufs[i]:
                distance += 1
        fintime = max(fintime, thisteam.workers[i].finishtime)
    return distance,fintime


#当装配过程中出现扰动时，给定相应的规则，重新将该班组的工序进行分配和排序

'''
    扰动1,2：无论发生何种扰动都采用NSGA2
'''
def dis_noregns(env, allstation, airs):
    #设置扰动的组有
    # 站位1，班组8，工人1；时间120，暂停70；
    # 站位5，班组11，工人2；时间1130，暂停10；
    #站位1，班组8，工人2；时间30；
    #站位1，班组7，工人2；30(第三类扰动)
    #站位1，班组7，工人2；40
    #站位1，班组1，工人2；50
    # 站位1，班组1，工人2；40
    # 站位1，班组4，工人2；90
    ...
    # 站位2，班组0，工人2；350
    # 站位2，班组5，工人2；450
    yield env.timeout(1130)
    air = airs[0]

    dis_station = 4
    dis_team = 15
    dis_worker = 1  ##忙碌的工人是2号工人
    now_action = 0
    ...
    now_rule = [0.416, 0.306, 0.095, 0., 0.166 ,0.163]
    # now_rule = [0.4,0.4,0.2,0.1,0.2,0.3]
    a,b = reschedule(allstation, airs, now_rule, dis_station, dis_team, dis_worker, orderbufs, fintimes, timepsts,islast=True)
    print(a,b)
'''
    扰动1,2：无论发生何种扰动都采用右移重调度
'''

def dis_noreg0(env, allstation, airs):
    #设置扰动的组有

    #站位1，班组8，工人1；时间100，暂停70；
    # 站位5，班组11，工人2；时间1130，暂停10；
    # 站位5，班组15，工人2；时间1133，暂停41；
    # 站位5，班组15，工人1；时间1200，暂停129；
    # 站位5，班组6，工人1；时间1120，暂停28；
    # 站位2，班组10，工人1；时间320，暂停111；
    # 站位2，班组10，工人2；时间390，暂停45；
    # 站位2，班组10，工人2；时间390，暂停20；
    # 站位2，班组10，工人3；时间350，暂停52；
    # 站位2，班组10，工人4；时间350，暂停18；
    # 站位1，班组15，工人1；时间260，暂停70；
    # 站位2，班组5，工人1；时间450，暂停100；
    # 站位2，班组5，工人3；时间500，暂停100；
    # 站位2，班组5，工人3；时间450，暂停150；
    # 站位1，班组4，工人3；时间80，暂停150；
    # 站位1，班组4，工人2；时间90，暂停150；
    # 站位2，班组15，工人2；时间390，暂停150；
    # 站位1，班组4，工人2；时间15，暂停139；
    #站位1，班组7，工人2；30(第三类扰动)
    #站位1，班组7，工人2；40
    #站位1，班组1，工人2；50
    # 站位1，班组1，工人2；40
...
def dis_reg(env, allstation, airs,states,dis_time,cur_time,dis_team=5,dis_worker=3):
    ...
    state = []
    state.append(dis_station)
    state.append(dis_team)
    state.append(len(thisorders)) ##工人的orderbuf是否为空
    state.append(reservetime) ##当前站位的剩余时间
    state.append(alltime)
    state.append(allposttime)
    state.append(allneighbortime) ##当受影响工序中有共同加工的工人，计算其他工人的装配序列时间和
    state.append(dis_time) ##扰动持续时间


    # state = np.array(state)
    # state = np.transpose(state)
    new_tensor = tf.expand_dims(state, axis=0)
    print(new_tensor)
    new_model = tf.keras.models.load_model('keras_model_hdf5_version.h5')
    y = new_model.predict(new_tensor)

    print(y)

def cal_time(env,airs,times):
    times.append(0)
    while True:
...
def ch1(env, allstation, airs,dis_station,dis_team,dis_worker):
    thisteam = allstation[dis_station].teams[dis_team]
    thisworker = thisteam.workers[dis_worker]
...
def ch2(env, allstation, airs,dis_station,dis_team,dis_worker):
    istwo = True
    # 开始时间
    ...
    for i in range(popnum):
        ...
        P[i].calculate_objective(allstation, airs, dis_station, dis_team, dis_worker, orderbufs, fintimes, timepsts,
                                 istwo)  # 计算目标函数值

    # 否 -> 非支配排序
    fast_non_dominated_sort(P)
    Q = make_new_pop(P, eta, bound_min, bound_max, allstation, airs, dis_station, dis_team, dis_worker, orderbufs,
                     fintimes, timepsts, istwo)
    ...
    for gen_cur in range(generations):
        ...
        Q_n = make_new_pop(P_n, eta, bound_min, bound_max, allstation, airs, dis_station, dis_team, dis_worker,
                           orderbufs, fintimes, timepsts,
                           istwo)  # use selection,crossover and mutation to create a new population Q_n
    ...
    now_rule = P_t[0].solution
    # now_rule = [0.4,0.4,0.2,0.1,0.2,0.3]
    reschedule1(allstation, airs, now_rule, dis_station, dis_team, dis_worker, orderbufs, fintimes, timepsts,islast=True)



def ch3(env, allstation, airs,dis_station,dis_team,dis_worker):
    ...
    for i in range(popnum):
        ...
        P[i].bound_process(bound_min, bound_max)  # 定义域越界处理
        P[i].solution = P[i].solution / sum(P[i].solution)
        P[i].calculate_objective(allstation, airs, dis_station, dis_team, dis_worker, orderbufs, fintimes,
                                 timepsts,istwo)  # 计算目标函数值
    ...
    for gen_cur in range(generations):
        ...
        Q_n = make_new_pop(P_n, eta, bound_min, bound_max, allstation, airs, dis_station, dis_team, dis_worker,
                           orderbufs, fintimes,
                           timepsts,istwo)  # use selection,crossover and mutation to create a new population Q_n
    ...
    plt.show()
    for ind in P_t:
        if ind.objective[2] < 9 and ind.objective[1] < 280:
            print("当前的解及其性能")
...
def col_information(allstation, dis_station, dis_team, dis_worker,aircaraft, states, dis_concretetime):
    air = aircaraft
...
def disturbance2(env, allstation, airs):
    ...
    col_information(allstation,dis_station,dis_team,dis_worker,air,states,dis_concretetime)
    print(states)

    cc = False
    thisteam = allstation[dis_station].teams[dis_team]
    for w in thisteam.workers:
        for oo in w.order_buffer:
            if dict_workernum[oo] == 5:
                cc = True

    dataset = pd.read_excel('总表.xlsx', header=0, index_col=0)
    ...
    test_state, _, _ = _normalize(states, train=False, specified_column=None, X_max=X_max, X_min=X_min)


    new_model = tf.keras.models.load_model('扰动分类模型.h5')
    changjings = new_model.predict(states)[0]
    changjing = list(changjings).index(max(changjings)) if not cc else 2
    print("当前场景是",changjing)

    if changjing == 0:
        ch1(env,allstation,airs,dis_station,dis_team,dis_worker)

    elif changjing == 1:
        ch2(env,allstation,airs,dis_station,dis_team,dis_worker)
    else:
        ch3(env, allstation, airs, dis_station, dis_team, dis_worker)
...
if __name__ == '__main__':
    ...
    for i in range(1):
        ...
        # env.process(dis_noreg0(env, allstation, all_aircraft))
        # env.process(dis_reg(env,allstation,all_aircraft,states,dis_time=250,cur_time=450))
        ...
        df_start.to_excel('D:/大论文数据/第四章/子策略验证1/开始时间3.xlsx')
        df_finish.to_excel('D:/大论文数据/第四章/子策略验证1/结束时间3.xlsx')
        df_statisworker.to_excel('D:/大论文数据/第四章/子策略验证1/工序集3.xlsx')
