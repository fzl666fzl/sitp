...
class Station:
    ...
    def calrule(self,air,nowrule):
        ...
        while air.orders_free:
            ...
            if self.id == 1:
                tmptt = self.teams[0]
                for www in tmptt.workers:
                    if www.stfi[0] > 480:
                        ffflag = True

            if self.id == 1 and dict_team[thisorder] == 0 and ffflag:
                worker_free = [2,3,4]
            ...
            dict_timet = 0
            if tw < 0.2:
                dict_timet = dict_time[thisorder]+dict_time[thisorder] * tw
            else:
                dict_timet =  dict_time[thisorder]

            if dict_free[thisorder] == 1:  ####初始的自由工序
                order_stfi[0] = latestfi
                order_stfi[1] = order_stfi[0] + dict_timet * maxefi
            else:
                tmp_maxtime = 0
            ...
            for k in range(thisnum):
                ...
                thisworkers[k].timelist[thisorder] = dict_timet * maxefi
                thisworkers[k].time_past += dict_timet * maxefi
