...
def disturbance2(env, allstation, airs):
    yield env.timeout(1050) ##9,420,1050
    air = airs[0]
    ...
    plt.show()
    for ind in P_t:
        if ind.objective[2] < 60 and ind.objective[1] < 560:
            print("当前的解及其性能")
...
if __name__ == '__main__':
    ...
    for i in range(1):
        ...
        thisteam = allstation[0].teams[11]
        for w in thisteam.workers:
            staticsworker[w.id] = " ".join(map(str, w.order_buffer[:]))
        staticsworker[5] = " ".join(map(str, staticslist[:]))
        print(staticsworker)
        ...
        # df_start.to_excel('D:/研三/大论文第四章数据/工序开始时间_495.xlsx')
        # df_finish.to_excel('D:/研三/大论文第四章数据/工序结束时间_495.xlsx')
        # df_statisworker.to_excel('D:/研三/大论文第四章数据/要展示的工序扰动495.xlsx')
