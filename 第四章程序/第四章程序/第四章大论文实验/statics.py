...
def disturbance(env, allstation, aircaraft, states, dis_concretetime):
    ...
    dis_time = random.randint(1450,1600)#扰动出现的时间，dis_concretetime为离开的时长
...
def disturbance20(env,allstation,airs):
    ...
    for w in freeworkers:
        ...
        if now_p in thisorders: ##说明正在加工的这道工序在等待别的工人，优先分配它，否则会死锁
            if dis_worker not in air.worker_list[now_p]:
...
if __name__ == '__main__':
    ...
    for i in range(50):
        ...
        dis_time = random.randint(10, 300) #扰动持续时间
        ...
        # df_start.to_excel('工序时刻表扰动20.xlsx')
        # df_finish.to_excel('工序结束时刻表扰动20.xlsx')
        # df_statisworker.to_excel('要展示的工序扰动20.xlsx')
    indexes = ["扰动站位", "班组号","待装配数量", "剩余时间", "受影响工序时间和", "受影响工序紧后时间和","邻居待装配时间和", "扰动持续时间","是否重调度"]
    df_states = pd.DataFrame(all_states,columns=indexes)
    df_states.to_excel("D:/研三/扰动数据说明/重调度数据收集扰动1_站位5.xlsx")
