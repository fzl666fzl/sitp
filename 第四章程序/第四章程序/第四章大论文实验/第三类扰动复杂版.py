...
class Worker:
    def __init__(self, id, env, station, profession):
        ...
        self.schedule = [] ##日程表，格式是[[st1,fi1],[st2,fi2]]
    ...
    def calrule(self, air):
        ...
        while air.orders_free:
            ...
            for k in range(thisnum):
                ...
                thisworkers[k].schedule.append([order_stfi[0],order_stfi[1]])
...
class Individual(object):
    ...
    def calculate_objective(self, allstation, airs,dis_station,dis_team,dis_worker,orderbufs,fintimes,timepsts,islast=False):
        objs0,objs1,a0,a1,b0,b1 = reschedule(allstation, airs, self.solution,dis_station,dis_team,dis_worker,orderbufs,fintimes,timepsts,islast)
        self.objective[1] = objs0
        self.objective[2] = objs1
...
def binary_tournament(ind1, ind2):
    ...
    # elif ind1.objective[1] > 281 and ind2.objective[1] < 281:
    #     return ind2
    # elif ind2.objective[1] > 281 and ind1.objective[1] < 281:
    #     return ind1
...
def disturbance2(env, allstation, airs):
    ...
    for ind in P_t:
        if ind.objective[2] < 20 and ind.objective[1] < 281:
        # if ind.objective[1] < 281:
            print("当前的解及其性能")
    ...
    objs0, objs1, a0, a1, b0, b1 = reschedule(allstation, airs,now_rule, dis_station, dis_team, dis_workers,
                                              orderbufs, fintimes, timepsts)
    zbid = [[dis_station+1,dis_team,1,a0,a1],[dis_station+1,dis_team,2,b0,b1]]
    print("招标书是",zbid)
    bids = resel_station(allstation, dis_station, dis_team,a0,a1,b0,b1)

    results = read_bid(bids)
    incomes = []
    for rr in results:
        rr1 = rr[:2]
        fintime = reschedule1(allstation, airs, now_rule, dis_station, dis_team, dis_workers, orderbufs, fintimes, timepsts,rr1,islast=True)
        incomes.append(fintime)
    big_income = incomes.index(max(incomes))
    print("最终选择的标书是",results[big_income])
    ...
    #
    # df_start.to_excel('D:/研三/大论文第四章数据/工序开始时间_fz22.xlsx')
    # df_finish.to_excel('D:/研三/大论文第四章数据/工序结束时间_fz22.xlsx')
    # df_statisworker.to_excel('D:/研三/大论文第四章数据/要展示的工序扰动fz22.xlsx')
...
def reschedule(allstation, airs, rule,dis_station,dis_team,dis_workers,orderbufs,fintimes,timepsts,islast=False):#rule是一个[0,0,0,0,0,0]指定了工序选择规则与工序分配规则
    ...
    return fintime,virtual_finishtime0-virtual_starttime0+virtual_finishtime1-virtual_starttime1,virtual_starttime0,virtual_finishtime0,virtual_starttime1,virtual_finishtime1



##用于查看不同效率的工人的
def reschedule1(allstation, airs, rule,dis_station,dis_team,dis_workers,orderbufs,fintimes,timepsts,effis,islast=False):#rule是一个[0,0,0,0,0,0]指定了工序选择规则与工序分配规则
    ##工人状态需要还原
    ...
    while freeo:
        ...
        if dict_free[new_o] == 1:  ####初始的自由工序
            ...
            if dis_workers[0] in new_w or dis_workers[1] in new_w:
                if dis_workers[0] in new_w:
                    order_stfi[1] = order_stfi[0] + dict_time[new_o]*effis[0]
                else:
                    order_stfi[1] = order_stfi[0] + dict_time[new_o] * effis[1]
            else:
                order_stfi[1] = order_stfi[0] + dict_time[new_o]
...
def resel_station(allstations,dis_station,dis_team,a0,a1,b0,b1):
    ...
    for s in free_stations:
        ...
        for w in range(worker_num):
            ...
            # if t_worker.is_pro and t_worker.now_pro < len(t_worker.order_buffer) - 1:
            #
            #     next_w = t_worker.order_buffer[t_worker.now_pro+1]
            #
            # else:
            #     if not t_worker.order_buffer and t_worker.efi < 1:
            #        if bidl < maxlen and t_worker.efi < 1:
            #            bidl += 1
            #            bid.append([dis_station,s,agr,1,w,t_worker.efi])
            if not t_worker.schedule:
                if bidl < maxlen and t_worker.efi < 1:
                   bidl += 1
                   bid.append([dis_station,s,agr,1,w,t_worker.efi])
                   bid.append([dis_station, s, agr, 2, w, t_worker.efi])
            else:
                tmpi = 0
                for ss in t_worker.schedule:
                    if ss[1] <= a0:
                        if tmpi == len(t_worker.schedule) - 1:
                            bid.append([dis_station, s, agr, 1, w, t_worker.efi])
                        elif tmpi < len(t_worker.schedule) - 1:
                            tmpss = t_worker.schedule[tmpi+1]
                            if tmpss[0] > a1:
                                bid.append([dis_station, s, agr, 1, w, t_worker.efi])
                    if ss[1] <= b0:
                        if tmpi == len(t_worker.schedule) - 1:
...
def read_bid(bids):
    ...
    # print("最终的组合是",[efi_b11,efi_b12,st_b11,st_b12],[efi_b21,efi_b22,st_b21,st_b22])

    return [efi_b11,efi_b12,st_b11+1,st_b12+1],[efi_b21,efi_b22,st_b21+1,st_b22+1]
...
if __name__ == '__main__':
    ...
    for i in range(1):
        ...
        thisteam = allstation[0].teams[13]
        for w in thisteam.workers:
            staticsworker[w.id] = " ".join(map(str, w.order_buffer[:]))
        staticsworker[5] = " ".join(map(str, staticslist[:]))
        print(staticsworker)
        ...
        # df_start.to_excel('E:/研三/大论文第四章数据/工序开始时间_fz2.xlsx')
        # df_finish.to_excel('E:/研三/大论文第四章数据/工序结束时间_fz2.xlsx')
        # df_statisworker.to_excel('E:/研三/大论文第四章数据/要展示的工序扰动fz2.xlsx')
