...
def disturbance2(env, allstation, airs):
    ...
    for i in range(popnum):
        ...
        P[i].calculate_objective(allstation, airs,dis_station,dis_team,dis_workers,orderbufs,fintimes,timepsts,istwo)  # 计算目标函数值

    # 否 -> 非支配排序
    fast_non_dominated_sort(P)
    Q = make_new_pop(P, eta, bound_min, bound_max,allstation, airs, dis_station,dis_team,dis_workers,orderbufs,fintimes,timepsts,istwo)
    ...
    for gen_cur in range(generations):
        ...
        Q_n = make_new_pop(P_n, eta, bound_min, bound_max,allstation, airs, dis_station,dis_team,dis_workers,orderbufs,fintimes,timepsts,istwo)  # use selection,crossover and mutation to create a new population Q_n
...
#扰动场景2下的实验
def disturbance22(env, allstation, airs):
    #站位1，班组2，时间350
    #站位2，班组0，时间350
    yield env.timeout(480)
    air = airs[1]
    ...
    for i in range(popnum):
        ...
        P[i].bound_process(bound_min, bound_max)  # 定义域越界处理
        P[i].solution = P[i].solution/sum(P[i].solution)
        P[i].calculate_objective(allstation, airs,dis_station,dis_team,dis_worker,orderbufs,fintimes,timepsts,istwo)  # 计算目标函数值

    # 否 -> 非支配排序
    fast_non_dominated_sort(P)
    Q = make_new_pop(P, eta, bound_min, bound_max,allstation, airs, dis_station,dis_team,dis_worker,orderbufs,fintimes,timepsts,istwo)
    ...
    for gen_cur in range(generations):
        ...
        F[i].sort(key=lambda x: x.distance)  # sort in descending order using <n，因为本身就在同一层，所以相当于直接比拥挤距离
        P_n = P_n + F[i][:popnum - len(P_n)]
        Q_n = make_new_pop(P_n, eta, bound_min, bound_max,allstation, airs, dis_station,dis_team,dis_worker,orderbufs,fintimes,timepsts,istwo)  # use selection,crossover and mutation to create a new population Q_n
...
# 集中式的计算完工时间和稳定性
def disturbance23(env, allstation, airs):
    yield env.timeout(480)
    # 开始时间
    start_time = time.time()
    air = airs[0]
    ...
    now_rule = [0.73, 0.72, 0.35, 0.16, 0.62, 0.10]
    ...
    dis,fin = reschedule23(allstation, airs, now_rule, dis_station, dis_team, dis_workers, orderbufs, fintimes, timepsts,islast=True)
    # reschedule(allstation, airs, now_rule, dis_station, dis_team, dis_workers, orderbufs, fintimes, timepsts,
    #            islast=True, effis[1])
...
def reschedule23(allstation, airs, rule,dis_station,dis_team,dis_workers,orderbufs,fintimes,timepsts,islast=False):#rule是一个[0,0,0,0,0,0]指定了工序选择规则与工序分配规则
    ##工人状态需要还原
...
if __name__ == '__main__':
    ...
    for i in range(1):
        ...
        # env.process((disturbance22(env, allstation, all_aircraft)))
        env.process((disturbance23(env, allstation, all_aircraft)))
