...
def disturbance2(env, allstation, airs):
    ...
    changjing = new_model.predict(states)
    ...
    #     P[i].calculate_objective(allstation, airs,dis_station,dis_team,dis_worker,orderbufs,fintimes,timepsts)  # 计算目标函数值
    #
    # # 否 -> 非支配排序
    # fast_non_dominated_sort(P)
    # Q = make_new_pop(P, eta, bound_min, bound_max,allstation, airs, dis_station,dis_team,dis_worker,orderbufs,fintimes,timepsts)
    #
    # P_t = P  # 当前这一届的父代种群
    # Q_t = Q  # 当前这一届的子代种群
    #
    ...
    #     Q_n = make_new_pop(P_n, eta, bound_min, bound_max,allstation, airs, dis_station,dis_team,dis_worker,orderbufs,fintimes,timepsts)  # use selection,crossover and mutation to create a new population Q_n
    #
...
def disturbance20(env, allstation, airs):
    ...
    now_rule = [0.6, 0.2,  0.1, 0.1, 0.14525334, 0.00295259]
    # now_rule = [0.4,0.4,0.2,0.1,0.2,0.3]
...
if __name__ == '__main__':
    ...
    for i in range(1):
        ...
        env.process((cal_time(env, all_aircraft,times)))
        for i in range(station_num):
        ...
        # df_start.to_excel('D:/研三/扰动数据说明/工序开始时间_nsgatmp.xlsx')
        # df_finish.to_excel('D:/研三/扰动数据说明/工序结束时间_nsgatmp.xlsx')
        # df_statisworker.to_excel('D:/研三/扰动数据说明/要展示的工序扰动nsgatmp.xlsx')
